...
#define TAOS_SENSOR_TMD2771_NAME	"tmd27711"
...
static int taos_als_threshold_set(void)
{
	...
	for (mcount = 0; mcount < 4; mcount++) {
		if ((ret = taos_write_byte(taos_datap->client,
			...
			printk(KERN_ERR"TAOS: write als failed in taos als threshold set\n");
			return (ret);
		}
	}
	return ret;
...
/*compare chip-id*/
	...
	if ((ret = taos_write_byte(taos_datap->client, TAOS_TRITON_CNTRL,
			     0x00)) < 0) {
		printk(KERN_ERR
			"%s: write the chip power down is failed\n", __func__);
	...
	if (taos_datap == NULL) {
		printk("[tmd27711] taos_datap == NULL\n");
	}
...
static void taos_late_resume(struct early_suspend *h)
{
	...
	if (taos_datap->working == 1) {
		...
		if ((ret = taos_write_byte(taos_datap->client, TAOS_TRITON_CNTRL, reg_store)) < 0) {
			printk(KERN_ERR
